    ...
    def make_file(self):
        if self._binary_file:
            ...
            return tempfile.NamedTemporaryFile(
                "w+", dir=args.directory, delete=False, encoding=self.encoding, newline="\n"
            )
...
class ListDirectoryInterception:
    ...
    def flush_headers_interceptor(self):
        for i, header in enumerate(self._headers_buffer):
            if header[:15] == b"Content-Length:":
                length = (
                    int(header[15:]) + len(DIRECTORY_BODY_INJECTION) + len(get_directory_head_injection(args.theme))
                )
    ...
    def copyfile_interceptor(self, source, outputfile):
        content = source.read()
        content = content.replace(b"</head>", get_directory_head_injection(args.theme) + b"</head>")
...
class SimpleHTTPRequestHandler(ListDirectoryInterception, http.server.SimpleHTTPRequestHandler):
    def do_GET(self):
...
class CGIHTTPRequestHandler(ListDirectoryInterception, http.server.CGIHTTPRequestHandler):
    def do_GET(self):
        if not check_http_authentication(self):
            return
...
def ssl_wrap(socket):
    ...
    if server_root in server_certificate.parents:
        print(f'Server certificate "{server_certificate}" is inside web server ' f'root "{server_root}", exiting')
        sys.exit(3)
    ...
    if args.client_certificate:
        ...
        if not client_certificate.is_file():
            print(f'Client certificate "{client_certificate}" not found, ' "exiting")
            sys.exit(4)

        if server_root in client_certificate.parents:
            print(f'Client certificate "{client_certificate}" is inside web ' f'server root "{server_root}", exiting')
            sys.exit(3)
...
def main():
    ...
    parser.add_argument("port", type=int, default=8000, nargs="?", help="Specify alternate port [default: 8000]")
    ...
    parser.add_argument(
        "--bind", "-b", metavar="ADDRESS", help="Specify alternate bind address [default: all interfaces]"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Specify a light or dark theme for the upload page " "[default: auto]",
    )
    parser.add_argument(
        "--server-certificate", "--certificate", "-c", help="Specify HTTPS server certificate to use [default: none]"
    )
    parser.add_argument(
        "--client-certificate", help="Specify HTTPS client certificate to accept for mutual TLS " "[default: none]"
    )
    parser.add_argument("--basic-auth", help="Specify user:pass for basic authentication (downloads and " "uploads)")
    parser.add_argument("--basic-auth-upload", help="Specify user:pass for basic authentication (uploads only)")
